<?php
    ...
    elseif(!empty($_POST['id_noiden'])){
        ...
        //Lấy loại xe
        $sql = mysqli_query($conn, "SELECT * FROM loai_xe INNER JOIN lich_chay ON loai_xe.id_loaixe =lich_chay.id_loaixe WHERE lich_chay.toLoc='$ten_noiden'");
        if(mysqli_num_rows($sql)==0){
            ...
            while($execute_loaixe=mysqli_fetch_array($sql)){
                echo '<option value="'.$execute_loaixe['id_loaixe'].'">'.$execute_loaixe['ten_loaixe'].'</option>';
            }
        } 
    }
?>
