<?php
    ...
    if(!empty($_POST['id_noiden'])){
        ...
        //Lấy tên tài xế
        $get_taixe = mysqli_query($conn, "SELECT * FROM nhanvien INNER JOIN noiden ON nhanvien.id_noidi = noiden.id_noidi WHERE noiden.id_noiden='$id_noiden' and nhanvien.fromLoc='Cần Thơ' and nhanvien.chucvu='Tài xế'");
        ...
        while($execute = mysqli_fetch_array($get_taixe)){
            ...
            if( $toLoc == $ten_noiden){
                $taixe = $execute['hoten'];
                echo '<option value="'.$taixe.'">'.$taixe.'</option>';
            }
        }
    }
}
